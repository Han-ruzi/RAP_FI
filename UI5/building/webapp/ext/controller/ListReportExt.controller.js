sap.ui.define([
    ...
    return {
        ...
        openExcelUpload: function(oEvent) {
            console.log(XLSX.version)
            var oView = this.getView();
            if (!this.pDialog) {
                Fragment.load({
                    id: "excel_upload",
                    name: "GJ.building.ext.fragment.ExcelUpload",
                    type: "XML",
                    controller: this
                }).then((oDialog) => { 
                    ...
                    .catch(error => alert(error.message));g
            } else {
                var oFileUploader = Fragment.byId("excel_upload", "uploadSet");
                oFileUploader.removeAllItems();
                this.pDialog.open();
        ...
        onTempDownload: function (oEvent) {
            console.log("Template Download Button Clicked!!!")
             ...
             var oBuilding = oModel.getServiceMetadata().dataServices.schema[0].entityType.find(x => x.name === 'Y_C_BUILDING_GJType');
             // set the list of entity property, that has to be present in excel file template
             var propertyList = ['BuildingId', 'BuildingName', 'NRooms', 'AddressLine','City', 'State', 'Country'];
        ...
        callOdata: function (fnResolve, fnReject) {
            ...
            this.excelSheetsData[0].forEach((value, index) => {
                ...
                oModel.create("/Y_C_BUILDING_GJ", payload, {
                    success: (result) => {
                        console.log(result);
